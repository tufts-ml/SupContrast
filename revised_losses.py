...
class DCLLoss(nn.Module):
    ...
    def forward(self, embeds: torch.Tensor, labels: torch.tensor):
        """Decoupled Contrastive Learning loss with cosine distance
        ...
        # construct denominator term for each numerator via logsumexp over a stack (B, B)
        log_denom = torch.zeros_like(logits)
        log_denom[in_numer] = base_denom[in_numer]  # no stacking of numerator from SINCERE
...
class MultiviewDCLLoss(DCLLoss):
    def __init__(self, temperature=0.07) -> None:
        super().__init__(temperature)
...
class LSELoss(nn.Module):
    ...
    def forward(self, embeds: torch.Tensor, labels: torch.tensor):
        """LogSumExp loss with cosine distance, maximizing logsumexp of target pairs and minimizing
        logsumexp of noise pairs
        ...
        # count numerator terms for averaging (B,)
        numer_count = in_numer.sum(dim=0)
        # logits is logsumexp of target pairs unlike SINCERE
        numer_logits_unaggregated = torch.full_like(logits, float("-inf"))  # (B, B)
        numer_logits_unaggregated[in_numer] = logits[in_numer]
        # numerator activations (B,), unlike (B, B) SINCERE because of target pair aggregation
        numer_logits = torch.zeros((logits.shape[0],))
        numer_logits = torch.logsumexp(numer_logits_unaggregated, dim=1)

        # construct denominator term for each numerator via logsumexp over a stack (B, B)
        # default value set to cancel with numer terms, unlike SINCERE
        log_denom = numer_logits.repeat(logits.shape[0], 1)
        log_denom[in_numer] = base_denom[in_numer]  # no stacking of numerator from SINCERE
...
class MultiviewLSELoss(LSELoss):
    def __init__(self, temperature=0.07) -> None:
        super().__init__(temperature)

    def forward(self, embeds, labels: torch.tensor):
        """LogSumExp loss with cosine distance and multiple image views
...
class LSESINCERELoss(nn.Module):
    def __init__(self, temperature=0.07) -> None:
        super().__init__()
        self.temperature = temperature

    def forward(self, embeds: torch.Tensor, labels: torch.tensor):
        """LogSumExp SINCERE loss with cosine distance, using logsumexp to aggregate target pairs
...
class MultiviewLSESINCERELoss(LSESINCERELoss):
    def __init__(self, temperature=0.07) -> None:
        super().__init__(temperature)

    def forward(self, embeds, labels: torch.tensor):
        """LogSumExp SINCERE with cosine distance and multiple image views
